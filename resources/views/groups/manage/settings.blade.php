<x-layouts.app title="Settings — {{ $group->name }}" description="Manage group settings.">
    <div class="mx-auto max-w-3xl px-4 py-10">
        <div class="mb-2">
            <a href="{{ route('groups.show', $group) }}" class="text-sm text-green-600 hover:text-green-700">&larr; Back to {{ $group->name }}</a>
        </div>

        <h1 class="text-2xl font-medium text-neutral-900">Group Settings</h1>
        <p class="mt-1 text-sm text-neutral-500">Update your group's information and preferences.</p>
        ...
        <form method="POST" action="{{ route('groups.manage.settings.update', $group) }}" enctype="multipart/form-data" class="mt-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            {{-- Slug --}}
            <div>
                <label for="slug" class="block text-sm font-medium text-neutral-700">URL Slug <span class="text-red-500">*</span></label>
                <div class="mt-1 flex items-center rounded-md border border-neutral-200 text-sm">
                    <span class="px-3 py-2 text-neutral-400 bg-neutral-50 border-r border-neutral-200 rounded-l-md">/groups/</span>
                    <input
                        type="text"
                        id="slug"
                        name="slug"
                        ...
                        required
                        class="block w-full px-3 py-2 text-neutral-900 placeholder-neutral-400 focus:border-green-500 focus:ring-1 focus:ring-green-500 focus:outline-none rounded-r-md border-0"
                    />
                </div>
                @error('slug')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                @if ($group->getFirstMediaUrl('cover_photo', 'card'))
                    <div class="mt-2 mb-2">
                        <img src="{{ $group->getFirstMediaUrl('cover_photo', 'card') }}" alt="Current cover photo" class="h-24 rounded-md object-cover" />
                    </div>
                @endif
                <input
                    type="file"
            ...
            <div>
                ...
                <div class="mt-2 space-y-2">
                    <label class="flex items-center gap-2">
                        <input type="radio" name="visibility" value="public" {{ old('visibility', $group->visibility->value) === 'public' ? 'checked' : '' }} class="text-green-500 focus:ring-green-500" />
                        <span class="text-sm text-neutral-700">Public — Anyone can find and view this group</span>
                    </label>
                    <label class="flex items-center gap-2">
                        <input type="radio" name="visibility" value="private" {{ old('visibility', $group->visibility->value) === 'private' ? 'checked' : '' }} class="text-green-500 focus:ring-green-500" />
            ...
            <div>
                <label class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="requires_approval" value="1" {{ old('requires_approval', $group->requires_approval) ? 'checked' : '' }} class="rounded border-neutral-200 text-green-500 focus:ring-green-500" />
            ...
            <div>
                ...
                <textarea
                    ...
                    class="mt-1 block w-full rounded-md border border-neutral-200 px-3 py-2 text-sm text-neutral-900 placeholder-neutral-400 focus:border-green-500 focus:ring-1 focus:ring-green-500 focus:outline-none"
                >{{ old('welcome_message', $group->welcome_message) }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let topics = @json(old('topics', $topics));
            ...
            function addQuestion(id, question, isRequired) {
                ...
                div.innerHTML =
                    '<div class="flex-1 space-y-2">' +
                        (id ? '<input type="hidden" name="membership_questions[' + index + '][id]" value="' + id + '" />' : '') +
            ...
            // Load existing membership questions
            @if(old('membership_questions'))
                @foreach(old('membership_questions') as $q)
                    addQuestion(@json($q['id'] ?? null), @json($q['question'] ?? ''), {{ !empty($q['is_required']) ? 'true' : 'false' }});
                @endforeach
            @else
                @foreach($group->membershipQuestions as $q)
                    addQuestion({{ $q->id }}, @json($q->question), {{ $q->is_required ? 'true' : 'false' }});
                @endforeach
            @endif
        });
    </script>
